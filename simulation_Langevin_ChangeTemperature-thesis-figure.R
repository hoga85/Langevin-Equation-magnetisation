...
while (EnterPara == T){
appliedB <- as.numeric(dlgInput("What is the strength of the AMF (unit: A/m) ?",
                     5000)$res)
appliedfreq <- as.numeric(dlgInput("What is the frequency of the AMF (unit: Hz) ?",
                        1)$res)
initialT <- as.numeric(dlgInput("What is the initial temperature of the MNPs (unit: K) ?",
                     50)$res)
finalT <- as.numeric(dlgInput("What is the final temperature of the MNPs (unit: K) ?",
                  350)$res)
dTemp <-  as.numeric(dlgInput("what is the incremental temperature (unit: K) ?",
                   100)$res)
...
#Domain setup
   ...
   #T <- 2 # the duration of time
   T<- (1/freq)*2
   #dt <- 0.001 #sampling time interval
   #n <- T/dt # data number per cycle
   n <- 1000
   dt <- T/n
   ...
   B0 <- appliedB #A/m
   B <- B0*sin(2*pi*freq*t)

   B0.all <- 10e3 # A/m; for MH curve
...
for (Temp in All.T ){
  ...
  ######
   ...
   col<-col+1
if (col ==1){
   plot(B.all[2:length(B)],M.all, type="l",lty = col,lwd=1, col = col)   
   grid()   
...
for (Temp in All.T ){
  ...
  if (col ==1){
    ...
    plot(t_M,M,  type="l",lty = col,lwd=1, col = col,
...
for (Temp in All.T ){
  ...
  if (col ==1){
    ...
    plot(B,t,  type="l",lty = col,lwd=1, col = col,
...
for (Temp in All.T ){
  ...
  if (col ==1){
    ...
    plot(f[1:(fft_data_length)],mag[2:(fft_data_length+1)], type="l",lty = col,lwd=1, col = col,
         xlim = c(0,6*freq))   
  }else{
